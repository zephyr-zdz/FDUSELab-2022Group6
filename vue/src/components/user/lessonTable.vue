  ...
  <el-card class="box-card">
    ...
    <el-table :data="lessonTable"
      ...
      <el-table-column
        ...
        width="100">
        <template v-slot="scope">
          <el-input size="mini" disabled v-show="(lessonEdit&&(scope.$index===editingIndex))" v-model="scope.row.teacher.jobnum" oninput="value=value.replace(/[^\d]/g,'')"
...
export default {
  ...
  methods: {
    ...
    getTeacher () {
      this.$axios.get('/api/teacher/teacher', {params: {jobnum: this.jobnum}})
        .then(response => {
    ...
    getLessons () {
      this.$axios.get('/api/teacher/course/jobnum', {params: {jobnum: this.jobnum}}).then(res => {
        this.lessonTable = res.data.data
      })
    },
    getSchool () {
      return this.teacher.institute
    },
    getTeacherId () {
    ...
    confirmRow (row, index) {
      ...
      this.$axios.post('/api/teacher/application/apply', this.application)
        .then(response => {
          if (response.data.code === 0) {
            this.$message({
              message: '修改课程申请成功',
    ...
    deleteRow (row, index) {
      ...
      this.$confirm('此操作将永久删除该课程, 是否继续?', '提示', {
        ...
        this.$axios.post('/api/teacher/application/apply', this.application).then(res => {
          if (res.data.code === 0) {
            this.getLessons()
            this.$message({
              message: '删除课程申请成功',
